...
const API_URL = process.env.REACT_APP_API_URL || 'https://cryso-backend.onrender.com';
...
function LoginPage({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(`✅ ${isRegister ? 'Registration' : 'Login'} successful:`, data.user);
...
export default function App() {
  ...
  useEffect(() => {
    const token = localStorage.getItem('cryso_token');
    if (token) {
      setIsLoading(true);
      fetch(`${API_URL}/api/auth/verify`, {
        ...
        .then(data => {
          if (data.valid) {
            ...
            localStorage.removeItem('cryso_token');
          }
        })
        .catch(() => localStorage.removeItem('cryso_token'))
